...
import { getPdfAnalisePedidoSienge } from "@/lib/sienge/client"
...
export async function GET(
  ...
  try {
    const pdf = await getPdfAnalisePedidoSienge(config.subdominio, config.usuario, senha, Number(pedidoId))
    return new NextResponse(new Uint8Array(pdf), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `inline; filename="analise-pedido-${pedidoId}.pdf"`,
      },
    })
  } catch (err) {
